...
def test_generator_plan_release_time_and_counts():
    ...
    g = Generator(env, agent_function=Agent, generation_mode="plan", production_plan=plan, plan_release_time=True)
    g.connections["next"] = q
    env.run(5)
    cnt = queue_count(q)
...
def test_generator_mix_and_batch():
    ...
    q = Queue(env, capacity=100)
    mix = [(A, 1), (B, 1)]
    g = Generator(env, agent_function=Agent, generation_mode="mix", production_mix=mix, batch_size=3)
    g.connections["next"] = q
    env.run(2)
    cnt = queue_count(q)
    assert cnt >= 3, f"Expected batch of 3 or more agents from mix, got {cnt}"
